...
const Likes = ({item}: {item: MediaItemWithOwner}) => {
  ...
  const handleLike = async () => {
    try {
      ...
      if (likeState.userLike) {
        ...
        await deleteLike(likeState.userLike.media_id, token);
        ...
        await postLike(String(item.media_id), token);
        getLikes();
        getLikeCount();
      }
    } catch (e) {
  ...
  return (
    <div className="justfy-center flex items-center">
      <label className="text-xl">{likeState.count}</label>
      <button className="p-2 text-2xl text-red-400" onClick={handleLike}>
        {likeState.userLike ? (
